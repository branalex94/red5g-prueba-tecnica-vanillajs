...
import { updateState, loadItems, updateArr } from './main.js'
import { modal } from './loadingModal.js'

const EVENTS = [
  {
    delegatorTag: '.table__body',
    targetTag: '.table-item__actions-button',
    event: 'click',
    handler: handleShowOptions
  },
  {
    delegatorTag: '.footer',
    targetTag: '.footer__actions-button',
    event: 'click',
    handler: handleFooterShowOptions
  },
  {
    delegatorTag: '.footer',
    targetTag: '.load-action',
    event: 'click',
    handler: handleClickItemsLoad
  },
  {
    delegatorTag: '.table__header',
    targetTag: 'th',
    event: 'click',
    handler: handleTableHeadingSortClick
  },
  {
    delegatorTag: '#results',
    targetTag: '',
    event: 'change',
    handler: handleResultsChange
  },
  {
    delegatorTag: '.right-arrow',
    targetTag: '',
    event: 'click',
    handler: handleRightArrowClick
  },
  {
    delegatorTag: '.left-arrow',
    targetTag: '',
    event: 'click',
    handler: handleLeftArrowClick
  },
  {
    delegatorTag: '.footer__search-input',
    targetTag: '',
    event: 'input',
...
// Function to map through events array and attach to elements
function events () {
  EVENTS.forEach(elem => {
    $(elem.delegatorTag).on(elem.event, elem?.targetTag, elem.handler)
  })
}
